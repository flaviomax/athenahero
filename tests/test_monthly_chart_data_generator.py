...
from athenahero.home.monthly_chart_data_generator import (
    data_read_by_day,
...
def seed_db_objects(session):  # noqa: F811
...
def test_empty_data_read_by_day(session):  # noqa: F811
    labels, values = data_read_by_day()
    assert labels == []
    assert values == []
...
def test_data_read_by_workgroup(seed_db_objects):
    ...
    assert results[0] == (
        "test_workgroup",
        bytes2gb(query_execution1.data_scanned_in_bytes + query_execution2.data_scanned_in_bytes),
    )
    assert results[1] == (
        "test_workgroup2",
        bytes2gb(query_execution3.data_scanned_in_bytes + query_execution4.data_scanned_in_bytes),
    )


def test_empty_data_read_by_workgroup(session):  # noqa: F811
    labels, values = data_read_by_workgroup()
    assert labels == []
    assert values == []
...
def test_empty_get_queries_data(session):  # noqa: F811
    most_expensive_queries = get_queries_data()
...
def test_empty_get_naive_queries_data(session):  # noqa: F811
    naive_queries = get_naive_queries_data()
    assert naive_queries["most_expensive_queries"] == []
    assert naive_queries["total_bytes_read"] == (None,)


def test_get_naive_queries_data_no_naives(session, seed_db_objects):  # noqa: F811
    query_execution1, query_execution2, query_execution3, query_execution4 = seed_db_objects
