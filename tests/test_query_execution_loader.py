...
from athenahero.query_execution_loader.query_execution_loader_job import (
    _get_successful_queries,
    _list_all_workgroups,
...
def boto_batch_get_query_execution_response(query_ids=None, overload_vals={}):
    ...
    payload = {"QueryExecutions": [get_query_execution_payload(query_id=i) for i in query_ids]}
...
def get_query_execution_payload(query_id=None, overload_vals={}):
    ...
    payload = {
        ...
        "ResultConfiguration": {"OutputLocation": f"s3://aws-athena-query-results-account-region/{_id}.csv"},
        "StatementType": "DML",
        "Statistics": {
...
def get_list_workgroups_response(overload_vals={}):
    payload = {
        "WorkGroups": [
            {"Name": "test_workgroup1", "State": "ENABLED", "Description": "", "CreationTime": datetime(2015, 1, 1)},
            {"Name": "test_workgroup2", "State": "ENABLED", "Description": "", "CreationTime": datetime(2015, 1, 1)},
        ]
    }
    return {**payload, **overload_vals}


def test_query_execution_parser(session):  # noqa: F811
...
def test_boto3_http_prevention(session):  # noqa: F811
    athena_client = boto3.client("athena")
    with pytest.raises(HTTPClientError):
        _list_all_workgroups(athena_client)


def test_workgroups_listing(session):  # noqa F811
    athena_client = boto3.client("athena")
    ...
    workgroups_second_response = get_list_workgroups_response(
        {
            "WorkGroups": [
                {"Name": "test_workgroup3", "State": "ENABLED", "Description": "", "CreationTime": datetime(2015, 1, 1)}
            ]
        }
    )
...
def test_successful_queries_filter(session):  # noqa: F811
    athena_client = boto3.client("athena")
...
def test_full_job(session):  # noqa: F811
    ...
    workgroup1_response["QueryExecutions"][10]["Status"]["CompletionDateTime"] = datetime.now(timezone.utc) - timedelta(
        days=90
    )
    for query_execution in workgroup1_response["QueryExecutions"]:
        query_execution["WorkGroup"] = "test_workgroup1"
    athena_stubber.add_response("batch_get_query_execution", workgroup1_response, {"QueryExecutionIds": ANY})
    ...
    athena_stubber.add_response("batch_get_query_execution", workgroup3_first_response, {"QueryExecutionIds": ANY})
    ...
    for query_execution in workgroup3_second_response["QueryExecutions"]:
        query_execution["WorkGroup"] = "test_workgroup3"
    athena_stubber.add_response("batch_get_query_execution", workgroup3_second_response, {"QueryExecutionIds": ANY})
    ...
    w1_queries = session.query(QueryExecution.id).filter(QueryExecution.workgroup == "test_workgroup1").all()

    assert w1_queries is not None
    assert len(w1_queries) == 49

    w2_queries = session.query(QueryExecution.id).filter(QueryExecution.workgroup == "test_workgroup2").all()

    assert w2_queries is not None
    assert len(w2_queries) == 0

    w3_queries = session.query(QueryExecution.id).filter(QueryExecution.workgroup == "test_workgroup3").all()
