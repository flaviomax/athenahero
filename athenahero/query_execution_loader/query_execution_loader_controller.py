...
query_execution_loader_bp = Blueprint("query_execution_loader_bp", __name__,)
...
def query_execution_loader():
    ...
    query_execution = QueryExecution(id=request_data["id"], query=request_data["id"])
    persist_query_execution(query_execution)

    response = make_response({"message": "All Done!"}, 200,)
